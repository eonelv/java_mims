 ...
 * 
 */
public class JFComboBoxUI extends BasicComboBoxUI
{

	protected JFComboBox comboBox;

	protected boolean hasFocus = false;

	private boolean isTableCellEditor = false;

	private static final String IS_TABLE_CELL_EDITOR = "JFComboBox.isTableCellEditor";
	...
	protected JFCellRendererPane currentValuePane = new JFCellRendererPane();
	 ...
	 * renderer when laying out and painting the "selected" item in the combo
	 * box. BasicComboBoxUI uses a single combo box renderer for rendering both
	 * the main combo box item and also all the items in the dropdown for the
	 * combo box. padding allows you to specify addition insets in addition to
	 * those specified by the cell renderer.
	 */
	...
	public void installUI(JComponent c)
	{
		...
		comboBox.setBackground(SwingConst.EDITOR_BACKGROUND);
		comboBox.setBorder(new JFBorders.TextFieldBorder());
	}
	...
	{
		return new JFComboPopup(comboBox);
	}
	...
	{
		JButton button = new JFArrowButton(SwingConstants.SOUTH);
	...
	public Dimension getMinimumSize(JComponent c)
	{
		...
		int buttonWidth = squareButton ? buttonHeight : arrowButton
	...
	protected Rectangle rectangleForCurrentValue()
	{
		...
		if (SwingUtils.isLeftToRight(comboBox))
		{
	...
	public void paintCurrentValue(Graphics g, Rectangle bounds, boolean hasFocus)
	{
		...
		currentValuePane.paintComponent(g, c, comboBox, x, y, w - 4, h,
				shouldValidate);
	}
	...
	protected void installKeyboardActions()
	{
		...
		LazyActionMap.installLazyActionMap(comboBox, JFComboBoxUI.class,
				"ComboBox.actionMap");
	}
	...
	private static class Actions extends UIAction
	{
		...
		{
			String key = getName();
			JFComboBox comboBox = (JFComboBox) e.getSource();
			JFComboBoxUI ui = (JFComboBoxUI) JFLookAndFeel.getUIOfType(
					comboBox.getUI(), JFComboBoxUI.class);
			if (key == HIDE)
			{
	...
	private class Handler implements ActionListener, FocusListener,
			KeyListener, LayoutManager, ListDataListener,
			PropertyChangeListener
	{
		...
		public void propertyChange(PropertyChangeEvent e)
		{
			...
			} else
			{
				...
				{
					updateToolTipTextForChildren();
				} else if (propertyName == JFComboBoxUI.IS_TABLE_CELL_EDITOR)
				{
		...
		public void keyPressed(KeyEvent e)
		{
			if (isNavigationKey(e.getKeyCode(), e.getModifiers()))
			{
				lastTime = 0L;
					...
					&& comboBox.getModel().getSize() != 0 && isTypeAheadKey(e)
		...
		public void layoutContainer(Container parent)
		{
			...
			if (arrowButton != null)
			{
				...
				buttonWidth = squareButton ? buttonHeight : arrowButton
						.getPreferredSize().width
						+ arrowInsets.left
						+ arrowInsets.right;
			}
			...
			if (arrowButton != null)
			{
				...
				{
					arrowButton.setBounds(width - (insets.right + buttonWidth) + 2
							, insets.top + 2, buttonWidth - 4,
							buttonWidth - 4);
//					arrowButton.setBounds(width - (insets.right + buttonWidth)
//							+ 2, insets.top + 1, buttonWidth - 2,
//							buttonHeight - 2);
				} else
				{
